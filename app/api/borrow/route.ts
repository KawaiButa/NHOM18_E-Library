      ...
      .then((response) => {
        if (response.status == 200)
        {
          ...
          data.forEach(element => {
              result.push(new BorrowForm(element._id, element.borrower._id,element.borrower.fullName, element.borrowDate, element.expectedReturnDate, element.isReturned))
          });
          console.log(result)
