            ...
            data.forEach((element, index) => {
              ...
              result.push(
                <tr>
                  <td>{index + 1}</td>
                  <td>{element.name}</td>
                  <td><FormControl type="number" size="sm" defaultValue={element.quantity} onChange={(event) => {
                    const temp = [...books]
                    var element = temp.at(index);
                    var amount = Number.parseInt(event.currentTarget.value)
                    if(element?.quantity && amount > 1)
                    {
              ...
              <Col className="d-flex justify-content-start">
                <Button
                  ...
                  onClick={async function HandleSummitEvent(event) {
                    ...
                    table?.append(child2);
                    const result:Array<{bookId:String, quantity:number}> = []
                    ...
                    await axios.patch("/api/borrow/" + id, {
                      method: "PATCH",
                      headers:{
                        'Content-Type': 'application/json', 
                      },
                      body: JSON.stringify(data)
                        ...
                        {
                          router.back();
                          alert("Update borrow form successfully")
                        }
                    }).catch((error) => {
                      alert(error.response.data)
                      router.refresh()
